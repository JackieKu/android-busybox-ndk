...
# ARCH for the the HOST that runs the toolchain
HOST_ARCH:=x86
...
#
ifeq ($(ARCH),arm)
  GCC_PREFIX:=$(ANDROID_NDK)/toolchains/arm-linux-androideabi-4.7/prebuilt/linux-$(HOST_ARCH)/bin/arm-linux-androideabi-
...
#
ifeq ($(ARCH),mips)
  GCC_PREFIX:=$(ANDROID_NDK)/toolchains/mipsel-linux-android-4.7/prebuilt/linux-$(HOST_ARCH)/bin/mipsel-linux-android-
...
#
ifeq ($(ARCH),x86)
  GCC_PREFIX:=$(ANDROID_NDK)/toolchains/x86-4.7/prebuilt/linux-$(HOST_ARCH)/bin/i686-linux-android-
  EXTRA_CFLAGS:=-DANDROID -D__ANDROID__ -DSK_RELEASE -nostdlib -fpic -fno-short-enums -fgcse-after-reload -frename-registers -Dhtons=__swap16 -Dhtonl=__swap32 -Dntohs=__swap16 -Dntohl=__swap32 -D_XOPEN_SOURCE -D_POSIX_C_SOURCE=200809
  EXTRA_LDFLAGS:=-Xlinker -z -Xlinker muldefs -nostdlib -Bdynamic -Xlinker -dynamic-linker -Xlinker /system/bin/linker -Xlinker -z -Xlinker nocopyreloc -Xlinker --no-undefined ${SYSROOT}/usr/lib/crtbegin_dynamic.o ${SYSROOT}/usr/lib/crtend_android.o
endif
...
patches: config
	...
	sed -e 's%\(CONFIG_CROSS_COMPILER_PREFIX=\).*%\1"$(GCC_PREFIX)"%' \
		-e 's%\(CONFIG_SYSROOT=\).*%\1"$(SYSROOT)"%' \
		-e 's%\(CONFIG_EXTRA_CFLAGS=\).*%\1"$(EXTRA_CFLAGS)"%' \
		-e 's%\(CONFIG_EXTRA_LDFLAGS=\).*%\1"$(EXTRA_LDFLAGS)"%' \
		$(CONFIG_FILE) \
		> $(BB_DIR)/.config
